...
UI 复刻和自动化流程编排不在本阶段继续投入。EasyCon 自动化所需控件作为兼容壳保存在 Upstream 的 `LegacyUi`，从而让 App 不直接依赖上游；所有新增业务能力均可在不创建窗口的情况下运行。

远程运行/停止、编译烧录、清除烧录、固件生成、脚本语法帮助、ESP32 配置和纯 UI 菜单行为均已从 App 入口和项目适配 API 中移除。`third_party` 中的实现仅作为上游源码快照保留。
